...
class memberModel extends Model
{
    ...
    public function getMemberGradeArr($show_progress = false, $exppoints = 0, $cur_level = '')
    {
        ...
        if ($member_grade && $show_progress) {
            if ($cur_level === '') {
                $cur_grade_arr = $this->getOneMemberGrade($exppoints, false, $member_grade);
                $cur_level = $cur_grade_arr['level'];
            }
    ...
    /**
     * @desc 将条件数组组合为SQL语句的条件部分
     * @author fzt
     * @date 2018-05-21
     * @param $conditon_array
     * @return string
     */
