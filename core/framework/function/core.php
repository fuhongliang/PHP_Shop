...
function Model($model = null, $base_path = null)
{
	...
	if(!empty($model) && !empty($_cache[$cache_key])) {
 ...
 * @date 2018-05-21
 * @author fzt
 * @param  $check_token bool 是否验证token
 * @param  $check_captcha bool 是否验证验证码
...
function chksubmit($check_token = false, $check_captcha = false, $return_type = 'alert'){
	...
	if ($check_captcha){
	    $nc_hash = empty($_POST['nchash']) ? '' : $_POST['nchash'];
	    $captcha = empty($_POST['captcha']) ? '' : $_POST['captcha'];
		if (!checkSeccode($nc_hash,$captcha)){
		    setNcCookie('seccode'.$nc_hash,'',-3600);
			if ($return_type == 'alert'){
				showDialog('验证码错误!');
			}else{
				return -12;
 ...
 * @return boolean
 */
function check_submit($check_token = false, $check_captcha = false, $return_type = 'alert'){
    $submit = isset($_POST['form_submit']) ? $_POST['form_submit'] : $_GET['form_submit'];
    if ($submit != 'ok') {
        return false;
    }
